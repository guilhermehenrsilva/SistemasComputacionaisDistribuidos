...
public class UDPEchoServer {
	private static byte[] BUFFER_OVER_FLOW_MESSAGE = "Dados acima do tamanho.".
			getBytes();
	...
	public static void start(int portNumber, int bufferSize)
		...
		try {
			...
			isRunning = false;
			throw new UDPEchoServerException(
					"Houve algum erro ao executar o servidor de eco UDP.",
					ioException);
		} finally {
			closeResources();
	...
	private static void run() throws IOException {
		System.out.printf("Servidor de eco rodando em ’%s:%d’ ...\n", InetAddress
				.getLocalHost().getHostAddress(), portNumber);
		isRunning = true;
		...
		while (true) {
			...
			if (dataIsUnderSize(received))
				...
				reply(received, BUFFER_OVER_FLOW_MESSAGE,
						BUFFER_OVER_FLOW_MESSAGE.length);
			}
		}
	}
	...
	private static void reply(DatagramPacket received, byte[] message,
			int bufferSize) throws IOException {
		DatagramPacket reply = new DatagramPacket(message, bufferSize,
				received.getAddress(), received.getPort());
		datagramSocket.send(reply);
		System.out.printf("Servidor respondendo %s:%d\n", received.getAddress()
				.getHostAddress(), received.getPort());
	}
