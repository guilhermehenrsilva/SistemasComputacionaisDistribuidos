	...
	public static void main(String[] args) {
		...
		try {
			while (!message.equals(KEY_TO_EXIT)) {
				...
				DataInputStream inputFlow = new DataInputStream(
						socket.getInputStream());
				DataOutputStream outputFlow = new DataOutputStream(
						socket.getOutputStream());
				// Envia a mensagem ao servidor (padr ̃ao UTF)
				outputFlow.writeUTF(message);
				// Lˆe a resposta do servidor
